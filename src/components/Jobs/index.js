...
class Jobs extends Component {
  ...
  getJobs = async () => {
    ...
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${employeeTypeList.join()}&search=${searchInput}&minimum_package=${minimumSalary}`
  ...
  renderJobsList = () => {
    ...
    const updatedList =
      locationList.length > 0
        ? jobsList.filter(eachJob => locationList.includes(eachJob.location))
        : jobsList
  ...
  handleLocation = location => {
    ...
    if (locationList.includes(location)) {
      const deletedItem = locationList.filter(each => each !== location)
      this.setState({locationList: deletedItem})
    } else {
